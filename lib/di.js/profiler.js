...
export function profileInjector(injector, Injector) {
  ...
  _global.di_report = function() {
    var data = _global.__di_dump__;
    var tokenIdMap = {};

    data.tokens.forEach((k, v) => {
      tokenIdMap[k] = v;
    });

    data.injectors.forEach((injector) => {
      console.log('Injector #' + injector.id);
      Object.keys(injector.providers).forEach((k) => {
        var provider = injector.providers[k];
        console.log('-- ' + provider.name);
        var deps = provider.dependencies;
        if (deps.length > 0) {
          deps.forEach((d) => {
            console.log('---- ' + tokenIdMap[d.token].name);
          });
        }
      });
    });
